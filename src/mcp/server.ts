...
const tools = [
  ...
  { name: "grok_image_generate", description: "Generate image(s). aspect_ratio 1:1/16:9/...; resolution 1k|2k; n batch; URLs temporary.", inputSchema: { type: "object", required: ["prompt"], properties: { prompt: { type: "string" }, model: { type: "string" }, n: { type: "number" }, aspect_ratio: { type: "string" }, resolution: { type: "string", description: "1k|2k" }, response_format: { type: "string" } } } },
  { name: "grok_image_edit", description: "Edit image with one source (URL/data). JSON only; single-image aspect follows source; multi-image not fully exposed.", inputSchema: { type: "object", required: ["prompt", "image_url"], properties: { prompt: { type: "string" }, image_url: { type: "string" }, model: { type: "string" }, n: { type: "number" }, response_format: { type: "string" } } } },
  { name: "grok_video_generate", description: "Generate video (async). duration 1-15s; aspect/resolution configurable. grok-imagine-video-1.5 is I2V-only (image_url required).", inputSchema: { type: "object", required: ["prompt"], properties: { prompt: { type: "string" }, model: { type: "string" }, duration: { type: "number", description: "1-15 seconds" }, aspect_ratio: { type: "string" }, resolution: { type: "string", description: "480p|720p|1080p" }, image_url: { type: "string", description: "Required for grok-imagine-video-1.5" } } } },
  { name: "grok_video_edit", description: "Edit video (async). Input must be .mp4 and <= ~8.7s. duration/aspect/resolution not customizable.", inputSchema: { type: "object", required: ["prompt", "video_url"], properties: { prompt: { type: "string" }, video_url: { type: "string", description: ".mp4, max ~8.7s" }, model: { type: "string" } } } },
